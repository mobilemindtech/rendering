...
abstract class ImageRenderingService extends RenderingService {
    ...
    static int DEFAULT_BUFFERED_IMAGE_TYPE = BufferedImage.TYPE_INT_ARGB

    protected abstract String getImageType()

    protected void configureRenderer(Graphics2DRenderer renderer) {
    ...
    protected convert(Map args, BufferedImage image, OutputStream outputStream) {
        ...
        if (!ImageIO.write(image, imageType, outputStream)) {
            throw new IllegalArgumentException("ImageIO.write() failed to find writer for type '$imageType'")
        }
    }
    ...
    protected BufferedImage createBufferedImage(Map args, Document document) {
        def bufferedImageType = args.bufferedImageType as Integer ?: getDefaultBufferedImageType()
        ...
        if (!renderHeight || autosizeWidth || autosizeHeight) {
            ...
            def tempGraphics = tempImage.createGraphics()
        ...
        if (args.scale) {
            scale(image, args.scale as Map, bufferedImageType)
        } else if (args.resize) {
            resize(image, args.resize as Map, bufferedImageType)
        } else {
            image
        }
    }
    ...
    @CompileDynamic
    protected BufferedImage scale(Image image, Map scaleArgs, bufferedImageType) {
    ...
    @CompileDynamic
    protected BufferedImage resize(BufferedImage image, Map resizeArgs, int bufferedImageType) {
    ...
    protected BufferedImage resize(BufferedImage image, int width, int height, int bufferedImageType) {
    ...
    protected BufferedImage scale(BufferedImage image, double widthScale, double heightScale, int bufferedImageType) {
        int width = (image.width * widthScale).toInteger()
        int height = (image.height * heightScale).toInteger()

        doScaleTransform(image, width, height, widthScale, heightScale, bufferedImageType)
    }

    protected BufferedImage doScaleTransform(BufferedImage image, int width, int height, double widthScale, double heightScale, int bufferedImageType) {
        ...
        graphics.drawRenderedImage(image as RenderedImage, transform)
        graphics.dispose()
